...
(** Create a new simulator for a given circuit.
    It will be only possible to [Instance.read] and [Instance.write] signals that are in [interesting_signals] or are inputs/outputs of the circuit. *)
...
val make_register_initialization_code : t -> string list
val make_memory_initialization_code : t -> string list
